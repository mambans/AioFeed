	...
	const handleResize = useCallback(
		(e) => {
			...
			if (resize.active) {
				...
				const newWidth = Math.max(window.innerWidth - (mouseX + (window.innerWidth - rightSideRefPosition.right)), MIN_WIDTH);
		...
		<>
			...
			<div ref={rightSideRef}>
				{React.Children?.map(children, (child) => {
					return React.isValidElement(child) ? React.cloneElement(child, { width: resize.width, style: { width: resize.width } }) : child;
				})}
			</div>

			{resize.active && <BackDrop transparent onMouseMove={handleResize} onMouseUp={handleMouseUp} />}
		</>
	);
};
