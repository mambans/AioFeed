...
export const fetchListVideos = async ({ list, currentVideos = [], videos }) => {
	if (videos || list?.videos) {
		...
		const twitchItems = (videos || list.videos).map((video) => typeof video === "number" && video).filter((i) => i);
		...
		const mergedVideosUnordered = [...(twitchItemsWithDetails || []), ...(youtubeItemsWithDetails || []), ...currentVideos];
...
const List = ({ list, setLists, setVideos, videos, savedVideosWithData, addSavedData, setLoading, loading, collapsed }) => {
	...
	useEffect(() => {
		(async () => {
			...
			setLoading(true);
			const videoIds = list?.videos.slice(0, (videosToShow?.showAll && list?.videos?.length) || Math.min(list?.videos?.length, videosToShow?.amount));
	...
	}, [list, setVideos, savedVideosWithData, addSavedData, setLoading, videosToShow?.amount, videosToShow?.showAll, collapsed]);
								...
								<>
									<ScrollArea
										...
										onMouseEnter={() => {
											...
											scrollInterval.current = setInterval(() => requestAnimationFrame(() => (wrapperRef.current.scrollLeft -= 15)), 3);
										}}
