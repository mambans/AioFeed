...
const FavoriteListContainer = ({ list, setLists, fetchMyListContextData, className, savedVideosWithData, addSavedData }) => {
	...
	return (
		<Container aria-labelledby={`MyList-${list.id}`} order={feedPreferences?.[list.id]?.order || 500} id={"MyListsHeader-" + list.id}>
			<Header
				id={list.title}
				title={
					<h1 id={`MyList-${list.id}`} onClick={() => togglePreference(list.id, "collapsed")}>
						{list.title}
							...
							items={[
								{
									...
									icon: list.enabled ? <AiFillEye size={22} color="#ffffff" /> : <AiOutlineEyeInvisible size={22} color="rgb(150,150,150)" />,
...
export const MyLists = () => {
	const { lists, setLists, fetchMyListContextData, savedVideosWithData, addSavedData } = useContext(MyListsContext);
			...
			{Boolean(Object.values(lists).length) ? (
				<TransitionGroup component={null}>
					{Object.values(lists)
						...
						?.map((list) => (
							<CSSTransition key={"mylist- " + list.id} timeout={1000} classNames="listHorizontalSlide" unmountOnExit appear>
				...
				<Alert type="secondary" title="No custom lists avaliable" message='Create your first "custtom list" in the sidebar' dismissible fill />
			)}
		</>
	);
};
...
const TriggerBtn = styled.button`
  ...
  transition: color 250ms, width 250ms;
	...
	background: ${({ open }) => (open ? "var(--popupListsBackground)" : "var(--refreshButtonBackground)")};
	text-align: left;
`;
