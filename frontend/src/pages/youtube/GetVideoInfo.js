...
const getVideoInfo = async ({ videos }) => {
	...
	const CachedFullyVideos = videosArray.filter((video) =>
		fullyCachedVideos.items.find((cache) => cache?.contentDetails?.upload?.videoId === video?.contentDetails?.upload?.videoId)
	);

	const unCachedFullyVideos =
		videosArray.filter(
			(video) => !fullyCachedVideos.items.find((cache) => cache?.contentDetails?.upload?.videoId === video?.contentDetails?.upload?.videoId)
		) || [];

	if (!Boolean(unCachedFullyVideos?.length)) {
		const cachedVideos = fullyCachedVideos.items.filter((cache) =>
			videosArray.find((video) => video?.contentDetails?.upload?.videoId === cache?.contentDetails?.upload?.videoId)
		);

		return cachedVideos;
	}

	const noVideoDetailsIDS = unCachedFullyVideos.map((video) => video?.contentDetails?.upload?.videoId);
	...
	const newVideosWithDetails =
		unCachedFullyVideos?.map((video) => {
			const newFoundDetails = newVideosDetails?.find((newDetails) => newDetails.id === video?.contentDetails?.upload?.videoId);
