...
import { ExpandCollapseFeedButton, LastRefreshText } from "../sharedComponents/sharedStyledComponents";
...
const SubFeedError = (props) => {
	...
	if (error && error.code) {
		switch (error.code) {
			case 401:
				alertError = error.errors[0].reason + " - Authendication expired and only cache used instead.";
				alertVariant = "danger";
				break;
			case 403:
		...
		return (
			<>
				<HeaderAlert key={error.errors[0].reason} variant={alertVariant || "warning"}>
					{alertError}
					{error.code === 401 && <ReAuthenticateButton serviceName={"Youtube"} style={{ marginLeft: "15px", display: "inline-block" }} />}
				</HeaderAlert>
			</>
		);
	} else {
...
const YouTubeHeader = (data) => {
	const { refresh, requestError, followedChannels, videos, isLoaded, setVideos, toggleExpanded, collapsed } = data;
	return (
		<Header
			id="YoutubeHeader"
			title={
