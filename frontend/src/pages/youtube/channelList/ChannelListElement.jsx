...
const ChannelListElement = (data) => {
	...
	return (
		<ChannelListLi key={channel.snippet.title} className={selected ? "selected" : ""} selected={selected}>
			...
			<ToolTip tooltip={`Unfollow ${channel.snippet.title}`}>
				<UnfollowButton disabled={getCookie("Youtube-readonly")} variant="link" onClick={handleUnfollow}>
					<MdDelete size={18} />
				</UnfollowButton>
			</ToolTip>
		</ChannelListLi>
