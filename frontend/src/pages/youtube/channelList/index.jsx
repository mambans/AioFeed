...
const ChannelList = (data) => {
	...
	const channels = useRef(getLocalstorage(`YT-followedChannels`) ? getLocalstorage(`YT-followedChannels`).data : []);
				...
				onChange: (event) => {
					...
					if (listIsOpen && event.target.value && event.target.value !== "") {
						const filtered = channels.current.filter((channel) => {
							return channel.snippet.title?.toLowerCase().includes((event.target.value || value)?.toLowerCase());
						});
	...
	const { value: channel, bind: bindChannel, reset: resetChannel, returnChannelId, manualSet } = useInput("");
	...
	useEffect(() => {
		if (listIsOpen || !channels.current) {
			channels.current = getLocalstorage(`YT-followedChannels`) ? getLocalstorage(`YT-followedChannels`).data : [];
			setFilteredChannels(getLocalstorage(`YT-followedChannels`) ? getLocalstorage(`YT-followedChannels`).data : []);
		}
	}, [listIsOpen]);
