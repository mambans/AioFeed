...
import { VideoContainer, VideoTitleHref, ImageContainer, ImgBottomInfo } from "../sharedComponents/sharedStyledComponents";
	...
	return (
		<VideoContainer key={video?.contentDetails?.upload?.videoId || video?.id} {...props}>
			<ImageContainer id={video?.contentDetails?.upload?.videoId} ref={ref} active={active}>
				<RemoveFromCurrentListButton videoId_p={video?.contentDetails?.upload?.videoId || video.id} list={list} />
				<AddToListButton videoId_p={video?.contentDetails?.upload?.videoId || video?.id} list={list} size={24} />
				{setPlayQueue && (
				...
				<Link
					...
					to={`/youtube/` + (video?.contentDetails?.upload?.videoId || video.id) + (listName ? `?list=${listName}` : "")}
				>
					<img
						src={
							video?.error
								? `${process.env.PUBLIC_URL}/images/youtube-not-found.png`
								: videoImageUrls(video?.snippet?.thumbnails) || `https://i3.ytimg.com/vi/${video.id}/maxresdefault.jpg`
						}
						alt=""
					/>
				</Link>
			...
			<ToolTip show={video?.snippet?.title?.length >= 50} tooltip={video?.snippet?.title} placement={"bottom"}>
				<VideoTitleHref error={!!video?.error} href={`https://www.youtube.com/watch?v=` + (video?.contentDetails?.upload?.videoId || video.id)}>
					{video?.snippet?.title || video.error}
				</VideoTitleHref>
			</ToolTip>
			<ChannelNameLink href={`https://www.youtube.com/channel/` + video?.snippet?.channelId}>{video?.snippet?.channelTitle}</ChannelNameLink>
			{video?.snippet?.publishedAt && <PublishedDate fromNow>{video?.snippet?.publishedAt}</PublishedDate>}{" "}
		</VideoContainer>
	);
});
