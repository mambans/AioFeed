...
const StyledDropdownContainer = styled.div`
	position: fixed;
	left: ${({ triggerOffset, selfOffets }) => triggerOffset?.left - (selfOffets?.width / 2 - triggerOffset?.width / 2)}px;
...
const DropdownContainer = ({ triggerRef, close, children }) => {
	...
	return (
		<StyledDropdownContainer ref={ref} triggerOffset={triggerOffset} selfOffets={selfOffets} onClick={close}>
			{children}
				...
				return (
					<div {...hoverEvents} tabIndex="5">
						...
						{portal(
							...
							>
								{items?.map(({ title, icon, onClick, color, destructive }, key) => {
									return (
										<ListActionButton key={key} color={color} onClick={onClick} destructive={destructive}>
											{icon}
											{title}
										</ListActionButton>
									);
