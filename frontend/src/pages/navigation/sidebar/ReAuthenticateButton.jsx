...
const ReAuthenticateButton = ({ disconnect, serviceName, style }) => {
	...
	const AuthButton = {
		Twitch: !twitchUsername ? (
			<ToolTip tooltip="Authenticate to Twitch">
				<StyledConnectTwitch id="connect" onClick={() => authenticatePopup("Twitch", `${TwitchBaseAuthUrl}&force_verify=true`)}>
					<FaTwitch size={24} />
					Connect Twitch
				</StyledConnectTwitch>
			</ToolTip>
			...
			<>
				<div className="username" id="Twitch">
					<div title="Re-authenticate" onClick={() => authenticatePopup("Twitch", `${TwitchBaseAuthUrl}`)}>
		...
		Youtube: !youtubeAccessToken ? (
			...
			<>
				<div className="username" id="Youtube">
					<div title="Re-authenticate" onClick={() => authenticatePopup("Youtube", `${YoutubeBaseAuthUrl}&prompt=consent`)}>
						<StyledReconnectIcon id="reconnectIcon" />
	...
	return <StyledConnectContainer style={{ ...style }}>{AuthButton[serviceName]}</StyledConnectContainer>;
};
