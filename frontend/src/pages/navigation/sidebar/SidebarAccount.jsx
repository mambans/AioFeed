...
const SidebarAccount = () => {
	...
	const { youtubeVideoHoverEnable, setYoutubeVideoHoverEnable, setYoutubeAccessToken, youtubeAccessToken } = useContext(YoutubeContext) || {};
	...
	const settingsButtons = [
		...
		{
			setEnable: () => togglePreference("twitch", "sidebar_enabled"),
		...
		},
		{
			...
			serviceName: "Twitch",
			tooltip: twitchAccessToken
				? (isEnabledUpdateNotifications ? "Disable " : "Enable ") + `notifications for when streams title or game changes`
				: `Need to connect/authenticate with a Twitch account first.`,
		...
		},
		{
			...
			serviceName: "Twitch",
			tooltip: twitchAccessToken
				? (isEnabledOfflineNotifications ? "Disable " : "Enable ") + `notifications for when streams go offline`
				: `Need to connect/authenticate with a Twitch account first.`,
		...
		<>
			<div style={{ minHeight: "calc(100% - 120px)" }}>
							...
							?.map((props, index) => {
								return (
									<ToggleButton
										...
										scrollIntoView
										setEnable={() => togglePreference(props.serviceName, "enabled")}
				...
				<SidebarExpandableSection title="Settings">
					<FeedSizeSlider />
					<ToggleButtonsContainer buttonsperrow={3}>
						{settingsButtons.map(({ setEnable, enabled, label, serviceName, tooltip, icon, smallerIcons, disabled }, index) => {
							return (
								<ToggleButton
					...
					<ReAuthenticateButton
						disconnect={() =>
							disconnectTwitch({
								setTwitchAccessToken,
								setEnableTwitch: () => togglePreference("twitch", "enabled"),
							})
						}
						serviceName="Twitch"
					/>
					<ReAuthenticateButton
						disconnect={() =>
							disconnectYoutube({
								...
								setEnableYoutube: () => togglePreference("youtube", "enabled"),
							})
						}
						serviceName="Youtube"
					/>
