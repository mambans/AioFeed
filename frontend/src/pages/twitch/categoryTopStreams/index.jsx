...
import { imageAspectDimensions } from "../../../utilities";
...
const TopStreams = () => {
	...
	const [videoType, setVideoType] = useState(type || URLQueries.get("type")?.toLowerCase() || p_videoType || "streams");
	const feedVideoSizeProps = useFeedVideoSizeObject();

	const [error, setError] = useState(null);
	const [sortBy, setSortBy] = useState("Views");
	const [sortByTime, setSortByTime] = useState(parseInt(URLQueries.get("within")?.toLowerCase()) || null);
	const oldTopData = useRef();
	const refreshBtnRef = useRef();
	const loadMoreRef = useRef();
	useDocumentTitle(`${gameInfo?.name || category || "All"} - ${videoType.charAt(0).toUpperCase() + videoType.slice(1)}`);
	...
	const fetchVideos = useCallback(
		(shouldLoadMore) => {
			setError(null);
			if (shouldLoadMore) loadMoreRef?.current?.setLoading?.(true);
	...
	return (
		<CSSTransition in={true} timeout={750} classNames="fade-750ms" appear>
			<Container>
						...
						<h1>
							...
							<div style={{ alignSelf: "end", textAlign: "start", fontSize: "0.95em" }}>
								...
								<span style={{ textTransform: "capitalize", fontSize: "0.85em" }}>
									{videoType}
									{!videoType || videoType === "streams" ? <MdLiveTv size={25} /> : <MdMovieCreation size={25} />}
								</span>
							</div>
						</h1>
					}
				...
				{error ? (
					<Alert variant="warning" style={{ textAlign: "center", width: "25%", margin: "auto" }} dismissible onClose={() => setError(null)}>
					...
					<TopStreamsContainer>
						<>
							<LoadingBoxes
								amount={Math.floor(((window.innerWidth - 150) / feedVideoSizeProps?.totalWidth) * 1.5)}
							...
							{!!topData?.length && <LoadMore onClick={() => fetchVideos(true)} ref={loadMoreRef} />}
						</>
					</TopStreamsContainer>
				)}
			</Container>
