...
const VideoPlayer = ({
	...
	useDocumentTitle(`${videoDetails?.user_name || channelName || ""} -  ${videoDetails?.title || videoId}`);
	...
	useEventListenerMemo(window?.Twitch?.Player?.PLAYING, OnPlayingEventListeners, twitchVideoPlayer.current, twitchVideoPlayer.current);
	useEventListenerMemo(window?.Twitch?.Player?.PLAYBACK_BLOCKED, onPlaybackBlocked, twitchVideoPlayer.current, twitchVideoPlayer.current);

	useEventListenerMemo(window?.Twitch?.Player?.PAUSE, OnPauseEventListeners, twitchVideoPlayer.current, twitchVideoPlayer.current);
	useEventListenerMemo(window?.Twitch?.Player?.READY, OnReadyEventListeners, twitchVideoPlayer.current, twitchVideoPlayer.current);
			...
			const fetchLatestVodId = async () => {
				const user =
					...
					(await TwitchAPI.getUser({
						...
						(res) =>
							res.data.data.find(
								(u) => u.display_name?.toLowerCase() === channelName?.toLowerCase() || u.login?.toLowerCase() === channelName?.toLowerCase()
							).id
					));
				...
				const video = await TwitchAPI.getVideos({
					...
					res.data.data.find(
						(v) => v?.user_name?.toLowerCase() === channelName?.toLowerCase() || v?.user_login?.toLowerCase() === channelName?.toLowerCase()
					)
				);
	...
	useEventListenerMemo(window?.Twitch?.Player?.ENDED, playNext, twitchVideoPlayer.current, twitchVideoPlayer.current);
				...
				if (twitchVideoPlayer.current) {
					...
					if (fetchedvideoDetails?.user_name && !channelName) {
						window.history.pushState(
							...
							`/${fetchedvideoDetails?.user_name}/videos/${videoId}${time ? `?start=${time}` : ""}${endTime ? `&end=${endTime}` : ""}${
	...
	const Qualities = () => {
		...
		return (
			<>
				{sourceQuality && (
					<div onClick={() => setPlayerQuality(sourceQuality.group)}>
				...
				>
					{qualities?.current
						...
						?.map((q, index) => (
							<div key={q.name || index} onClick={() => setPlayerQuality(q.group)}>
								{q.name}
							</div>
						)) || null}
				</ContextMenuDropDown>
	...
	return (
		<>
			...
			<Loop listIsOpen={String(listIsOpen)} listWidth={listWidth} loopEnabled={String(loopEnabled)}>
				{twitchVideoPlayer.current && (
					<ToolTip placement="right" delay={{ show: 500, hide: 0 }} tooltip={`${loopEnabled ? "Disable " : "Enable "} loop`} width="max-content">
						<LoopBtn
							size={32}
							...
							onClick={() => {
								setLoopEnabled((cr) => {
									...
									window.history.pushState({}, `${window.location}${loopEnabled ? `&loop=${loopEnabled}` : ""}`);
									return !cr;
								});
							}}
						/>
				...
				{twitchVideoPlayer.current && loopEnabled && (
					<Loopbar twitchVideoPlayer={twitchVideoPlayer.current} duration={duration} loopEnabled={loopEnabled} />
				)}
