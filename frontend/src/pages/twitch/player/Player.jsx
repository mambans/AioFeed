...
const Player = () => {
		...
		if (twitchVideoPlayer.current) {
			const LIVEStreamInfo = {
				...
				...(await fetchStreamInfo(
					twitchVideoPlayer.current.getChannelId() ? { user_id: twitchVideoPlayer.current.getChannelId() } : { user_login: channelName }
				)),
			};
	...
	return (
		...
		>
			...
			{!chatState.hideChat && (
				<>
					<Chat streamInfo={streamInfo} channelName={channelName} chatState={chatState} updateChatState={updateChatState} />

					{resizeActive && <ChatOverlay onMouseUp={handleResizeMouseUp} onMouseMove={resize} switched={chatState.switchChatSide} />}
				</>
			)}
		</VideoAndChatContainer>
	);
