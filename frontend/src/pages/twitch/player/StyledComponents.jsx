...
export const VideoAndChatContainer = styled.div`
	...
	grid-template-areas: ${({ switchedChatState, hidechat, chatAsOverlay }) =>
		hidechat || chatAsOverlay ? '"video"' : switchedChatState === "true" ? '"chat devider video"' : '"video devider chat"'};
	...
	grid-template-columns: ${({ chatwidth, hidechat, switched, chatAsOverlay }) =>
		`${hidechat || chatAsOverlay ? "100vw" : switched ? `${chatwidth}px min-content auto` : `auto min-content ${chatwidth}px`} `};
...
export const StyledVolumeEventOverlay = styled.div`
	...
	width: ${({ type, hidechat, chatwidth, chatAsOverlay, isFullscreen }) =>
		hidechat === "true" || chatAsOverlay === "true" || isFullscreen === "true"
			...
			: "98vw"};
	height: ${({ vodVolumeOverlayEnabled }) => (vodVolumeOverlayEnabled ? `calc(100% - ${VEO_margin_bottom}px)` : "100%")};
	...
	margin: auto;
	margin-top: "auto";
	margin-bottom: ${({ vodVolumeOverlayEnabled }) => (vodVolumeOverlayEnabled ? VEO_margin_bottom + "px" : "auto")};
...
export const ChatWrapper = styled.div`
	...
	height: ${({ chatAsOverlay, overlayPosition }) => (chatAsOverlay ? (overlayPosition.height ? overlayPosition.height + "px" : "50%") : "100%")};
	width: ${({ chatAsOverlay, overlayPosition }) => (chatAsOverlay ? (overlayPosition.width ? overlayPosition.width + "px" : "unset") : "unset")};
