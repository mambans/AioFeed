...
const PlayPauseButton = ({ TwitchPlayer, PlayerUIControlls }) => {
	...
	useEventListenerMemo(window?.Twitch?.Player?.PLAY, () => setIsPaused(false), TwitchPlayer, window?.Twitch?.Player?.READY);
	useEventListenerMemo(window?.Twitch?.Player?.PAUSE, () => setIsPaused(true), TwitchPlayer, window?.Twitch?.Player?.READY);
	...
	const button = (() => {
		if (isPaused) {
			return <FaPlay disabled={!TwitchPlayer} id="PausePlay" size={30} onClick={PausePlay} title={"Play (space)"} />;
		}
		return <FaPause disabled={!TwitchPlayer} id="PausePlay" size={30} onClick={PausePlay} title={"Pause (space)"} />;
	})();
