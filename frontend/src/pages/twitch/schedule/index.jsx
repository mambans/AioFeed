	...
	return (
		<>
			<Container absolute={String(absolute)}>
				{!alwaysVisible && <TriggerButton setShow={setShow} ref={btnRef} absolute={String(absolute)} btnSize={btnSize} style={style} />}
				{(show || alwaysVisible) && (
					<Portal>
						<RefreshBtn
							ref={refreshBtnRef}
			...
			if (!schedule) {
				...
				if (Boolean(gameIds?.filter((i) => i).length)) {
					...
					fetchedSchedule.data.data.segments.map(
						(s) => (s.category["box_art_url"] = gameData?.data?.data?.find((g) => g.id === s.category?.id)["box_art_url"])
					);
				}
