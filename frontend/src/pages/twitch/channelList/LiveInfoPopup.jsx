	...
	return (
		<Container>
			<div id="title">
				...
				<div>
					<img id="game" src={channel?.game_img?.replace("{width}", 130)?.replace("{height}", 173)} alt="" />
					{channel?.game_name}
				</div>
			</div>
			<img src={channel?.thumbnail_url?.replace("{width}", 1280)?.replace("{height}", 720) + `#` + Date.now()} alt="" />
			<div id="bottom">
				<div>
					<FaRegClock size={9} />
