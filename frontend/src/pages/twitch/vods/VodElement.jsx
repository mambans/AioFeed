...
const VodElement = React.memo(
	({ data, vodBtnDisabled, setDragSelected, listName, list, active, setPlayQueue, playQueue, size, loading, ...props }) => {
		const {
			id,
			user_id,
			login,
		...
		return (
			<VideoContainer draggable={Boolean(setDragSelected)} onDragStart={onDragStart} {...props}>
				<ImageContainer ref={imgRef} active={active}>
					...
					<AddToListButton videoId_p={id} style={{ top: thumbnail_url === "" ? "30px" : "0px" }} size={24} list={list} />
					{setPlayQueue && <AddRemoveFromPlayQueueButton videoId={id} setPlayQueue={setPlayQueue} playQueue={playQueue} />}
					...
					<ImgBottomInfo>
						<div title="duration">{thumbnail_url === "" ? <Moment durationFromNow>{created_at}</Moment> : formatTwitchVodsDuration(duration)}</div>
				...
				<ChannelContainer>
					...
					<ChannelNameDiv>
						...
						<VodsFollowUnfollowBtn show={!vodBtnDisabled} channel={data} loweropacity="0.5" className="extaButton" />
					</ChannelNameDiv>
					{created_at && (
						<VodDates>
							<div>
								...
								>
									{`${moment(created_at).format("dd HH:mm")} → ${
										thumbnail_url === "" ? moment(Date.now()).format("dd HH:mm") : moment(endDate).format("dd HH:mm")
									}`}
								</p>
							</div>
						</VodDates>
