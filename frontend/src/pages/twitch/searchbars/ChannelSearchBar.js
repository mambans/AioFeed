      ...
      try {
        ...
        if (query) {
          if (query !== previousSearch.current || page) {
            ...
            if (searchResults?.data?.data?.length > limit)
      ...
      [
        ...(followedChannels || [])?.filter(
          (i) =>
            ...
            loginNameFormat(i)
              ?.toLowerCase()
              ?.includes(inputRef.current.value?.trimStart?.()?.trim?.()?.toLowerCase())
        ),
        ...(vodChannels || [])?.filter(
          (i) =>
            // new RegExp(inputRef.current?.value?.trim?.()?.toLowerCase(), 'i').test(loginNameFormat(i))
      ...
      <InputWrapper>
        ...
        {searchButton && (
          <SearchBarSuffixButton
            onClick={onSubmit}
            disabled={inputRef.current !== document.activeElement || !search?.trim?.()}
          >
            <SearchSubmitIcon />
          </SearchBarSuffixButton>
        )}
