...
const Handler = ({ children }) => {
	...
	const { newlyAddedStreams, setLiveStreams } = useStreamsStore();
	...
	const [documentTitle, setDocumentTitle] = useDocumentTitle(newlyAddedStreams?.length ? `(${newlyAddedStreams?.length}) Feed` : "Feed");
		...
		async ({ firstLoad = false } = {}) => {
			...
			try {
				...
				if (Array.isArray(baseStreams)) {
					...
					const tag_ids = streamsTag_ids.filter((tag) => !streamTags.current.find(({ tag_id } = {}) => tag_id === tag));
					...
					const fetchedStreamTags = uniqueTags?.length
						...
						: [];
					const tags = uniqBy([...(streamTags.current || []), ...(fetchedStreamTags || [])], "tag_id");
