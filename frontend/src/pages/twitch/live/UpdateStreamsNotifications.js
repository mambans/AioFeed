...
export const checkUpdatedStreams = ({ streams, updateNotischannels }) => {
  return streams
    ?.filter((stream) => updateNotischannels?.includes(stream.user_id))
    ?.map((stream) => {
      const { title, body } = (() => {
        if (stream.title !== stream.oldData?.title && stream.game_id !== stream.oldData?.game_id) {
          return {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const updatedStreams = checkUpdatedStreams({
          streams: newUpdatedNonFeedSectionStreams,
          updateNotischannels,
        });
