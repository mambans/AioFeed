...
import { baseLiveStreamsAtom, feedSectionsAtom, previousBaseLiveStreamsAtom } from "../twitch/atoms";
			...
			{Object.values(feedSections)
				...
				?.map((section, index) => (
					<CSSTransition key={section.id} timeout={1000} classNames="listHorizontalSlide" unmountOnExit appear>
						<Section key={section.id} section={section} index={index} data={section.data} addNotification={addNotification} />
					</CSSTransition>
				))}
		</TransitionGroup>
	);
			...
			try {
				if (previosStreams?.current && data?.loaded) {
					const streamsToNotifyLive = notifications_enabled
						? feedSectionStreams?.filter((stream) => !previosStreams?.current?.find?.((s) => s?.user_id === stream?.user_id))
						: [];
					...
					const streams = streamsToNotifyLive?.map?.((s = {}) => {
						...
						if (fetchLatestVod && wentLive) {
							timer.current = setTimeout(() => fetchLatestVod({ user_id: stream.user_id, check: true }), 30000);
						}
					...
					const leftStreams = streamsToNotifyLeftSection?.map((s = {}) => {
						...
						const notisTitle = wentOffline ? `${loginNameFormat(s)} went offline from ${title}` : `${loginNameFormat(s)} left ${title}`;
						const stream = { ...s, notiStatus: notisTitle };
						...
						addSystemNotification({
							...
							onClick: (e) => {
								e.preventDefault();
								const url = `https://aiofeed.com/${loginNameFormat(stream, true)?.toLowerCase()}/page`;
								window.open(url, "_blank");
							},
						});
